...
export default class AllQuestionsController {
  constructor(
    ...
    const getClassName = function(useEditQuestion) {
      ...
      if (
        useEditQuestion &&
        typeof $scope.questionToEdit.className !== "undefined"
      ) {
    ...
    $scope.importQuestions = function() {
      ...
      questions.import($scope.questionsToImport).then(newQuestions => {
        ...
        const message =
          "You have successfully imported new questions! Scroll to the bottom of the page to see.";
    ...
    $scope.editQ = function(questionIndex) {
      ...
      questions
        .editQuestion($scope.questionToEdit._id, $scope.questionToEdit)
        .then(() => {
    ...
    $scope.deleteQuestion = index => {
      const conf = window.confirm(
        "Are you sure you want to permanantly delete that question?"
      );
      if (conf === true) {
