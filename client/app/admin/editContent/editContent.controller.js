'use strict';
angular.module('bitfit').controller('EditContentCtrl', function ($scope, $http, Auth, User, socket, topics, topic, topicPromiseEC, $location, Flash) {
  ...
  $scope.addHintToExistingQ = function () {
    $scope.questionToEdit.hints.push('');  //$scope.topic.questions[questionIndex].hints.push("");
  };
  ...
  $scope.aceLoaded = function (_editor) {
    ...
    console.log($scope.editor);  // Events
  ...
  $scope.deleteQuestion = function (id, index) {
    ...
    $scope.topic.questions.splice(index, 1);  //$http.delete('/api/questions/' + id);
  };
