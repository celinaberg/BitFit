  ...
  constructor(
    ...
    $scope.compileCode = function() {
      ...
      if (typeof code === "undefined" || code === "") {
        $scope.output.compileOutput =
          "In order to compile your program, please enter code in the code editor.\n";
        return;
      }
      ...
      if (fileName === ".c") {
        $scope.output.compileOutput =
          "In order to compile your program, please enter a name for your C file.\n";
        return;
      }
      const obj = {
        className,
    ...
    $scope.checkAnswer = function() {
      ...
      if (typeof code === "undefined" || code === "") {
        $scope.showComments = false;
        $scope.output.compileOutput =
          "In order to check your answer, please enter code in the code editor.\n";
        return;
      }
      $http.post("api/clis/compile", obj).then(res => {
        if (res.data === "") {
        ...
        $http.post("api/clis/run", obj).then(res => {
          if (typeof res.data === "object") {
            ...
            if (
              $scope.output.expectedOutput.trim() ===
              $scope.output.runOutput.trim()
            ) {
              $scope.feedback = "Well done!";
              logging.progress.correctAttempts++;
            } else {
              $scope.feedback =
                "Your output doesn't quite match the output we're looking for. Please try again\n";
            }
          }
        });
      });
    ...
    $scope.showHint = function() {
      ...
      if (
        ...
        if (
          $scope.status[$scope.qInfo.currentQuestion - 1].hintIndex <
          $scope.topic.questions[$scope.qInfo.currentQuestion - 1].hints.length
        ) {
    ...
    $scope.pageChanged = function() {
      ...
      logging.progress.question =
        $scope.topic.questions[$scope.qInfo.currentQuestion - 1]._id;
    ...
    $scope.updatePageWithNewQuestion = function() {
      ...
      if (
        typeof $scope.topic.questions === "undefined" ||
        typeof $scope.topic.questions[$scope.qInfo.currentQuestion - 1] ===
          "undefined"
      ) {
        return;
      }
      const currQuestion =
        $scope.topic.questions[$scope.qInfo.currentQuestion - 1];

      // Set starter code, if this question has any
      if (typeof currQuestion.code !== "undefined") {
        $scope.editor.setValue(
          $scope.topic.questions[$scope.qInfo.currentQuestion - 1].code,
          -1
        ); // -1 is document start
      }
    ...
    $scope.init = function() {
      ...
      if ($scope.topic.questions.length > 0) {
        logging.progress.question =
          $scope.topic.questions[$scope.qInfo.currentQuestion - 1]._id; // for first question only
        logging.progress.startTime = Date.now();
      }
    };
