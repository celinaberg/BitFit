'use strict';
angular.module('bitfit').controller('TopicsCtrl', function($scope, $stateParams, $location, $http, Auth, topics, topic, topicPromiseTC, logging) {
  // topics is for manipulating questions
  ...
  $scope.compileCode = function() {
    ...
    $http.post('api/clis/compile', obj).then(function(data) {
      if (data.data === '') {
        ...
        $scope.output.compileOutput = data.data;
      }
    });
  ...
  $scope.runCode = function() {
    ...
    $http.post('api/clis/run', obj).then(function(data) {
      if (typeof data.data === 'object') {
        ...
        $scope.output.runOutput = data.data;
      }
    });
    ...
    $http.post('api/clis/compile', obj).then(function(data) {
      ...
      $http.post('api/clis/run', obj).then(function(data) {
        if (typeof data.data === 'object') {
          // Likely an error
          $scope.showComments = false;
          $scope.output.runOutput += $scope.handleError(data.data);
        } else {
          $scope.output.runOutput = data.data;
  ...
  $scope.pageChanged = function() {
    ...
    logging.progress.endTime = Date.now();
    logging.logProgress();
    // async so this returns immediately
  ...
  $scope.updatePageWithNewQuestion = function() {
    ...
    if (typeof $scope.topic.questions === 'undefined' || typeof $scope.topic.questions[$scope.qInfo.currentQuestion - 1] === 'undefined') {
      return;
    }
